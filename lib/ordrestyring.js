...
export async function fetchGpsViaCases(caseNumbers) {
  ...
  const argName = field.args?.[0]?.name || "caseId";
  const argType = unwrap(field.args?.[0]?.type) || "Int";
  const retType = unwrap(field.type) || "";
  const wrapItems = /Pagination/i.test(retType);
  debug.push(`gpsEntriesForCase(${argName}: ${argType}) -> ${retType}`);
  ...
  for (const nr of numbers) {
    ...
    const query = wrapItems
      ? `query { gpsEntriesForCase(${argName}: ${val}) { items { ${GPS_FIELDS} } } }`
      : `query { gpsEntriesForCase(${argName}: ${val}) { ${GPS_FIELDS} } }`;
...
export async function fetchGpsViaCars(days = 7) {
  const debug = [];
  const now = Math.floor(Date.now() / 1000);
  const from = now - days * 24 * 3600;

  // Hvordan ser gpsEntries ud? (arg-navne + er svaret pakket ind?)
  const intro = await callGraphql(
    `query { __type(name: "Query") { fields { name args { name type { kind name ofType { kind name ofType { name } } } } type { kind name ofType { kind name ofType { name } } } } } }`,
    {}
  );
  if (intro.error) return { error: intro.error, debug: debug.join(" | ") };
  const field = (intro.data?.__type?.fields || []).find((f) => f.name === "gpsEntries");
  if (!field) return { error: "gpsEntries findes ikke i API'et.", debug: debug.join(" | ") };
  const unwrap = (t) => (t ? t.name || unwrap(t.ofType) : null);
  const argNames = (field.args || []).map((a) => a.name);
  const carArg = argNames.find((n) => /^car$/i.test(n)) || "car";
  const timeArg = argNames.find((n) => /time/i.test(n)) || "startTime";
  const wrapItems = /Pagination/i.test(unwrap(field.type) || "");
  debug.push(`gpsEntries(${argNames.join(", ")})${wrapItems ? " [items]" : ""}`);

  // Bilerne
  const carsResult = await callGraphql(`query { cars { name } }`, {});
  if (carsResult.error) return { error: carsResult.error, debug: debug.join(" | ") };
  if (carsResult.graphqlError)
    return { error: `cars fejlede: ${carsResult.graphqlError}`, debug: debug.join(" | ") };
  const names = (carsResult.data?.cars || []).map((c) => c?.name).filter(Boolean);
  debug.push(`${names.length} biler`);
  if (!names.length) return { entries: [], via: "cars", debug: debug.join(" | ") };

  const entries = [];
  let serverBug = false;
  for (const name of names) {
    const safe = String(name).replace(/"/g, '\\"');
    const sel = wrapItems ? `{ items { ${GPS_FIELDS} } }` : `{ ${GPS_FIELDS} }`;
    const query = `query { gpsEntries(${carArg}: "${safe}", ${timeArg}: {from: ${from}, to: ${now}}) ${sel} }`;
    const r = await callGraphql(query, {});
    if (r.graphqlError) {
      if (/SQLSTATE/i.test(String(r.graphqlError))) {
        serverBug = true;
    ...
    const list = wrapItems ? r.data?.gpsEntries?.items : r.data?.gpsEntries;
    if (Array.isArray(list) && list.length) {
      entries.push(...list);
...
export async function fetchGpsSmart(caseNumbers = []) {
  ...
  const perCar = await fetchGpsViaCars(7);

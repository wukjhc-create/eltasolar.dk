...
async function taskFields(formData) {
  ...
  const fields = {
    ...
    was_returned: status === "tilbage" ? true : undefined,
...
export async function updateTask(formData) {
  ...
  if (error) throw new Error(error.message);
  // Status og tilbage-aarsag gaelder hele sagen
  if (fields.order_number) {
    await client
      ...
      .update({
        ...
        ...(fields.was_returned ? { was_returned: true } : {}),
...
export async function setTaskStatus(formData) {
  ...
  const { data: task } = await client
    .from("tasks")
    .select("order_number,return_reason")
    .eq("id", id)
    .single();
  const fields = {
    status,
